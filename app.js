console.log("✅ App OK - V10 bedre UX");
...
function setStatus(text, type = "") { const box = el("status"); box.textContent = text; box.className = "status " + type; }
...
function toggleAdmin() { el("adminPanel").classList.toggle("hidden"); renderAuthUi(); }
function makeId(prefix) { return prefix + "_" + Date.now() + "_" + Math.random().toString(36).substring(2, 7); }
function generatePin() { return String(Math.floor(1000 + Math.random() * 9000)); }
function sortedEntries(obj) { return Object.entries(obj || {}).sort((a, b) => (a[1].order || 0) - (b[1].order || 0)); }
function saveLocal() { localStorage.setItem("players", JSON.stringify(players)); localStorage.setItem("items", JSON.stringify(items)); localStorage.setItem("checks", JSON.stringify(checks)); }
function loadLocal() { players = JSON.parse(localStorage.getItem("players") || "{}"); items = JSON.parse(localStorage.getItem("items") || "{}"); checks = JSON.parse(localStorage.getItem("checks") || "{}"); }
function playerUrl(playerId) { return location.origin + location.pathname + "?spiller=" + encodeURIComponent(playerId); }
function copyText(text) { navigator.clipboard.writeText(text).then(() => showToast("Kopiert ✅", "ok")).catch(() => prompt("Kopier teksten:", text)); }
...
function ensureAuth() {
  ...
  return auth.signInAnonymously().then(res => res.user).catch(err => {
...
function write(path, value, adminOnly = false) {
  ...
  if (db) {
    return ensureAuth().then(() => db.ref(path).set(value)).then(() => {
      if (adminOnly) showToast("Lagret ✅", "ok");
    }).catch(err => userFriendlyFirebaseError(err, "Firebase write-feil"));
  }
  applyLocal(path, value); saveLocal(); render(); showToast("Lagret lokalt ✅", "ok");
}

function removePath(path, adminOnly = false) {
  if (adminOnly && !requireAdmin()) return;
  if (db) {
    return ensureAuth().then(() => db.ref(path).remove()).then(() => {
      showToast("Slettet ✅", "ok");
  ...
  saveLocal(); render(); showToast("Slettet lokalt ✅", "ok");
}
...
function applyLocal(path, value) {
  ...
  if (parts[0] === "checks") { checks[parts[1]] = checks[parts[1]] || {}; checks[parts[1]][parts[2]] = value; }
...
function loginAnon() { ensureAuth().then(() => { renderAuthUi(); showToast("Bruker anonympålogging", "ok"); }); }
function logoutAdmin() { if (!auth) return; auth.signOut().then(() => { currentUser = null; isAdmin = false; renderAuthUi(); showToast("Logget ut", "ok"); }); }
...
function lockSelectedPin() { if (!selectedPlayerId) return; delete unlockedPlayers[selectedPlayerId]; localStorage.setItem("unlockedPlayers", JSON.stringify(unlockedPlayers)); setStatus("🔒 PIN låst for valgt spiller", "warn"); render(); }
...
function selectPlayer() { selectedPlayerId = el("playerSelect").value; localStorage.setItem("selectedPlayerId", selectedPlayerId); askPin(selectedPlayerId); render(); }
function showAllPlayers() { history.replaceState(null, "", location.origin + location.pathname); urlPlayerId = ""; el("directBox").classList.add("hidden"); el("playerSelect").disabled = false; render(); }
function toggleCheck(itemId, checked) { if (!selectedPlayerId) return; if (!askPin(selectedPlayerId)) return; write(`checks/${selectedPlayerId}/${itemId}`, checked, false); }
function copyPlayerLink(playerId) { const player = players[playerId]; copyText(`${player.name}: ${playerUrl(playerId)}  PIN: ${player.pin || "ingen"}`); }
function copySelectedPlayerLink() { if (!selectedPlayerId || !players[selectedPlayerId]) return; copyPlayerLink(selectedPlayerId); }
function copyAllLinks() { const lines = sortedEntries(players).map(([id, p]) => `${p.name}: ${playerUrl(id)}  PIN: ${p.pin || "ingen"}`); copyText(lines.join("\n")); }
function deletePlayer(playerId) { if (!confirm("Slette spiller og avkrysninger?")) return; removePath("players/" + playerId, true); removePath("checks/" + playerId, false); }
function deleteItem(itemId) { if (!confirm("Slette pakkepunkt?")) return; removePath("items/" + itemId, true); }
function savePlayerEdit(playerId) {
  if (!requireAdmin()) return;
  const existing = players[playerId] || {};
  write("players/" + playerId, { ...existing, name: el("name_" + playerId).value.trim() || "Uten navn", pin: el("pin_" + playerId).value.trim(), order: existing.order || Date.now() }, true);
}
...
function seedDefaultItemsIfEmpty(force = false) {
  ...
  defaultItems.forEach((name, index) => {
    ...
    write("items/" + id, { name, order: index + 1 }, force);
  });
}

function render() { updateAdminStatus(); renderDirectMode(); renderPlayers(); renderPackingList(); renderAdmin(); renderAuthUi(); }
function renderDirectMode() { const direct = !!urlPlayerId; el("directBox").classList.toggle("hidden", !direct); el("playerSelect").disabled = direct; }
function renderPlayers() {
  const select = el("playerSelect"); select.innerHTML = "";
  const entries = sortedEntries(players);
  if (entries.length === 0) { select.innerHTML = '<option value="">Ingen spillere lagt til</option>'; selectedPlayerId = ""; el("playerInfo").innerHTML = "Legg til spiller i Admin-panelet."; return; }
  ...
  entries.forEach(([id, player]) => { const option = document.createElement("option"); option.value = id; option.textContent = player.name; option.selected = id === selectedPlayerId; select.appendChild(option); });
...
function renderPackingList() {
  const div = el("packingList"); div.innerHTML = "";
  if (!selectedPlayerId || !players[selectedPlayerId]) { div.innerHTML = '<p>Velg eller legg til spiller først.</p>'; return; }
  if (!hasPinAccess(selectedPlayerId)) { div.innerHTML = '<p>Denne listen er låst med PIN. Skriv PIN for å se og krysse av.</p>'; return; }
  const entries = sortedEntries(items);
  if (entries.length === 0) { div.innerHTML = '<p>Ingen pakkepunkter. Legg til i Admin.</p>'; return; }
  entries.forEach(([itemId, item]) => {
    const checked = !!(checks[selectedPlayerId] && checks[selectedPlayerId][itemId]);
    const row = document.createElement("label"); row.className = "packItem" + (checked ? " done" : "");
    const checkbox = document.createElement("input"); checkbox.type = "checkbox"; checkbox.checked = checked; checkbox.onchange = () => toggleCheck(itemId, checkbox.checked);
    const span = document.createElement("span"); span.textContent = item.name;
    row.appendChild(checkbox); row.appendChild(span); div.appendChild(row);
  });
}
function renderAdmin() {
  const adminPlayers = el("adminPlayers"); const adminItems = el("adminItems"); adminPlayers.innerHTML = ""; adminItems.innerHTML = "";
  sortedEntries(players).forEach(([id, p]) => {
    const row = document.createElement("div"); row.className = "adminRow playerEdit";
    row.innerHTML = `<input id="name_${id}" value="${String(p.name || "").replace(/"/g, '&quot;')}" aria-label="Navn"><input id="pin_${id}" value="${String(p.pin || "").replace(/"/g, '&quot;')}" aria-label="PIN">`;
    ...
    row.appendChild(copyBtn); row.appendChild(pinBtn); row.appendChild(saveBtn); row.appendChild(delBtn); adminPlayers.appendChild(row);
  });
  sortedEntries(items).forEach(([id, item]) => {
    const row = document.createElement("div"); row.className = "adminRow"; row.innerHTML = `<span>${item.name}</span>`;
    const delBtn = document.createElement("button"); delBtn.textContent = "Slett"; delBtn.className = "danger"; delBtn.onclick = () => deleteItem(id);
    row.appendChild(delBtn); adminItems.appendChild(row);
  });
}
...
function init() {
  ...
  if (auth) {
    auth.onAuthStateChanged(user => { currentUser = user; updateAdminStatus(); renderAuthUi(); render(); });
    ensureAuth();
  }
  if (db) {
    ...
    setStatus("Kjører lokalt uten Firebase", "warn"); render(); seedDefaultItemsIfEmpty(false);
  }
}
init();
